...
from database.mongodb import add_exercise
...
def sync_metrics_update(context):
    ...
    if target_sets > 0 and reps_per_set > 0 and sets_completed > last_saved_sets:
        ...
        now_ts = time.time()
        started_st = st.session_state.get("set_cycle_started_at", now_ts)
        time_taken = now_ts - started_st
        ...
        if st.session_state.get("voice_pipeline"):
            result = st.session_state.voice_pipeline.process_event(
                event="set_completed",exercise = exercise,
